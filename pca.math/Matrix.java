...
public class Matrix implements Cloneable, java.io.Serializable {
   ...
   /*
	...
	public Matrix ReNorm() {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				A[i][j] = (A[i][j] - min[j])/(max[j] - min[j]) * 255;
			}
		}
		return this;
	}
	 ...
	 * One norm
	 * 
	 * @return maximum mean a new copy.
	 */

	public Matrix meanPCACopy() {
		Matrix X = new Matrix(m, n);
		double[][] C = X.getArray();
		double[] temp = new double[n];
		for (int j = 0; j < n; j++) {
	...
	/**
	 * 
	 * turn the matrix horizon
	 */

	public Matrix turnhorizon() {
		for (int i = 0; i < m; i++) {
			double temp = 0;
			for (int j = 0; j <= n/2; j++) {
				temp = A[i][n - j -1];
